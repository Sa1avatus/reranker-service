...
- Never log or persist input text on the server, bearer credentials, or un-hashed cache inputs.
  - Browser-local input persistence is enabled by default for Playground and Batch Playground;
    users can opt out via the "Remember inputs in this browser" checkbox.
